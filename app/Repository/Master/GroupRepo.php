...
class GroupRepo
{
    ...
    public function saveData(GroupKknRequest $groupKknRequest): JsonResponse
    {
        try {
            ...
            // check group exist
            $id_dpl = $groupKknRequest->id_dpl;
            $id_desa = $groupKknRequest->id_desa;
            $groupexist = DB::table("tbl_group_anggota_kkn")->whereRaw("id_dpl=?", [$id_dpl])->whereRaw("id_desa=?", [$id_desa])->first();
            if ($groupexist) {
                $idgroup = $groupexist->id;
            } else {
            ...
            foreach ($data as $key => $value) {
                ...
                $calon = DB::table("tbl_calon_kkn")->where("id_calon_kkn", $value["id_calon_kkn"])->update([
                    "group" => 1,
                ]);
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function unassignGroup(int $id): JsonResponse
    {
        try {
            DB::beginTransaction();
            $exist = DB::table("tbl_detail_anggota_kkn")->whereRaw("id=?", [$id]);
            $id_calon_kkn = $exist->first()->id_calon_kkn;
            DB::table("tbl_calon_kkn")->whereRaw("id_calon_kkn=?", [$id_calon_kkn])->update(
                [
                    "group" => 0,
                ]
            );
